...
const Realworld = () => {
  ...
  },
  {
    ...
    projects: [
      ...
      },
      {
        ...
        description: "Developed grocery delivery platform with cart & checkout.",
        image: "/how-we-work.png",
  ...
  const getProjectsToShow = () => {
    ...
    const found = projectCategories.find(cat => cat.name === selectedCategory);
  ...
  return (
    <div className='realworld'>
      ...
      <ul className="category-list">
        {categories.map((name) => {
          const count = name === "All Work"
            ? allProjects.length
            : projectCategories.find(c => c.name === name)?.projects.length || 0;
          return (
            <li
              key={name}
      ...
      <div className="work-area">
        {projectsToShow.map((project, index) => (
          <div className="work-div" key={index} style={{ backgroundImage: `url(${project.image})` }}>
            <div className="work-overlay">
              <p><span/>{project.name}</p>
              <p>{project.description}</p>
              <div className="detail-btn">Show Detail</div>
            </div>
          </div>
        ))}
      </div>
