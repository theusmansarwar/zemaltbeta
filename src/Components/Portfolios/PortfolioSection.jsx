...
const PortfolioSection = () => {
  ...
  useEffect(() => {
    const container = scrollContainerRef.current;
    if (!container || portfolios.length === 0 || !container.children.length)
      return;
  ...
  return (
    <>
      <div className="portfolio-section" id="portfolio">
        ...
        <p>
          Great work begins with the right people and institutions. Zemalt
          brings together experts who know how to turn ideas into measurable
          results. Every project starts with a clear plan, a proven strategy,
          and a skilled team that works with precision. Our specialists bring
          mastery in SEO, ads, content, and design. Each member plays a role
          that strengthens campaigns and builds momentum.
        </p>
        ...
        <div className="portfolio-cards-container" ref={scrollContainerRef}>
          {isLoading
            ? [...Array(4)].map((_, i) => (
                <PortfolioSkeleton key={`skeleton-${i}`} />
              ))
        ...
        <div className=" port-dots-wrapper">
          {Array.from({ length: portfolios.length - visibleCards + 1 }).map(
            (_, index) => (
              <span
                key={index}
